...
// Add Code Potent's Update Manager
...
class SendPosition {
	...
	public function register_script() {
		wp_register_script('smyp-script', plugins_url('/scripts/smyp.js', __FILE__), [], '1.0.1', true);
		wp_register_style('smyp-style', plugins_url('/styles/smyp.css', __FILE__), [], '1.0.1', 'all');
	}
	...
	public function button_shortcode($atts, $content = null) {
		...
		if ($wa === false) {
			...
			if (current_user_can('edit_posts')) {
				$message = esc_attr__('Please check your [smyp] shortcode. The "wa" attribute must be set and must be in international format (only you can see this message).', 'smyp').'<a href="'.get_edit_post_link().'">'.esc_html__('Edit post.', 'smyp').'</a>';
			}
			return $message;
		}
